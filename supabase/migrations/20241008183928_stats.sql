drop function if exists "public"."get_general_chart_labels"(p_currency currency_type, p_month integer, p_year integer);

drop function if exists "public"."get_general_stats"(p_currency currency_type, p_month integer, p_year integer);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.get_stats_data(p_currency currency_type, p_month integer DEFAULT EXTRACT(month FROM CURRENT_DATE), p_year integer DEFAULT EXTRACT(year FROM CURRENT_DATE))
...
begin
  ...
  left join public.operations o on
    (o.issued_at::date between date_trunc('month', make_date(p_year, p_month, 1)) and c1.date) and
...
CREATE OR REPLACE FUNCTION public.get_stats_expenses_by_label(p_currency currency_type, p_month integer DEFAULT EXTRACT(month FROM CURRENT_DATE), p_year integer DEFAULT EXTRACT(year FROM CURRENT_DATE))
...
begin
  ...
  with cte1 as (
    ...
    where 
      ...
      extract(month from e.issued_at) = p_month and
      extract(year from e.issued_at) = p_year
    group by e.label
  ...
  cte2 as (
    ...
    where 
      e.currency = p_currency and
      extract(month from e.issued_at) = p_month and
      extract(year from e.issued_at) = p_year and
...
CREATE OR REPLACE FUNCTION public.get_stats_operations_by_day_of_week(p_currency currency_type, p_month integer DEFAULT EXTRACT(month FROM CURRENT_DATE), p_year integer DEFAULT EXTRACT(year FROM CURRENT_DATE))
...
begin
  ...
  left join operations o
    on cast(extract(dow from o.issued_at) as integer) = d.day_of_week
    and o.currency = p_currency
    and extract(month from o.issued_at) = p_month
    and extract(year from o.issued_at) = p_year
